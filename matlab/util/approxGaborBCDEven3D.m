function [x, approximator, gaborSigma, gaborFT] = ...
                    approxGaborBCDEven3D(n, sigmaDenom, z0Denom, nloops)
...
% approximated
    ...
    initFun = @(x) sum(sum(sum((gaborFT - (exp(-((R-z0)/(n/x(1))).^2)).* ...
                (polyval([x(5), x(4), x(3), x(2)], (R-z0))) .* ...
                (polyval([x(7), 0, x(6), 0,0], cosPhi))).^2,3), 2), 1);
    xinit = [sigmaDenom, 1, 0, 0, 0, 0 0];
    ...
    for iloop = 1:nloops
        ...
        polyRfunc = @(a) sum(sum(sum((gaborFT-(exp(-((R-z0)/(n/sigma)).^2)) ...
                 .* (polyval([a(4), a(3), a(2), a(1)], (R-z0))) .* ...
                (polyval([b(2), 0, b(1), 0, 0], cosPhi))).^2,3), 2), 1);
        ainit = [x(2), x(3), x(4), x(5)];
        ...
        gaborSigma = x; 
        x = [sigma, a(1), a(2), a(3), a(4), b(1), b(2)];
        assert(numel(x) == 7)
        fval
    end
    gaborFT = 0.5 * exp( -((X).^2+Y.^2+(Z-z0).^2)/gaborSigma^2) + ...
